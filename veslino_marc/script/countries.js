    ...
    .then((data) => {
      ...
      return fetch(`https://restcountries.com/v3.1/region/${region}`)
        ...
        .then(regionData => {
          ...
          regionData.sort((a, b) => a.name.common.localeCompare
            (b.name.common));
            ...
            if (country.name.common.toLowerCase() !== currentCountry) {
              regionCountriesHTML += `
                <div class="region-country"
                  ...
                  class="small-flag"><p>${country.name.common}</p>
                </div>`;
            }
          });

          regionContainer.innerHTML = regionCountriesHTML ||
            `No other countries found in this region.`;
        });
    })
    .catch((error) => {
      console.error(error);
